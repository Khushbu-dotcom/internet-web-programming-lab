...
function calculateTotal($expenses, $selectedDate) {
    ...
    foreach ($expenses as $expense) {
        $parts = explode("|", trim($expense));

        if (count($parts) == 3 && trim($parts[0]) == $selectedDate) {
            $total += (float)$parts[2];
        }
    }
...
foreach ($expenses as $expense) {
    ...
    if (count($parts) == 3 && trim($parts[0]) == $today) {
